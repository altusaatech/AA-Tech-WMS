...
export const salesBom = pgTable(
  "sales_bom",
  {
    ...
    // Real BOM Status fields (the module previously reused the SO columns).
    gaDrawingsFolderLink: text("ga_drawings_folder_link"),
    bomFolderLink: text("bom_folder_link"),
    bomStatus: text("bom_status"),
    reasonsForDelay: text("reasons_for_delay"),
    bomRelatedNotes: text("bom_related_notes"),
    bomAmendmentNeeded: boolean("bom_amendment_needed").notNull().default(false),
    bomAmendmentReasons: text("bom_amendment_reasons"),
    bomAmendmentDate: date("bom_amendment_date"),
    noOfDays: integer("no_of_days"),
    bomTargetDate: date("bom_target_date"),
    bomActualDate: date("bom_actual_date"),
    bomNo: text("bom_no"),
    createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
  },
