...
export function NavArrows() {
  ...
  const iconBtn =
    "group inline-flex size-9 items-center justify-center rounded-xl text-slate-500 transition-all duration-200 hover:-translate-y-0.5 hover:bg-[#0180cf]/10 hover:text-[#0069b3] active:scale-90";
  return (
    <div className="fixed left-4 bottom-4 z-[130] flex items-center gap-1 rounded-2xl border border-white/80 bg-white/90 p-1.5 shadow-[0_16px_36px_-12px_rgba(15,40,80,0.42),inset_0_1px_0_rgba(255,255,255,0.9)] backdrop-blur-xl print:hidden max-md:left-3 max-md:bottom-3">
      ...
      <button type="button" onClick={() => router.back()} aria-label="Go back" title="Back" className={iconBtn}>
        <ChevronLeft size={18} strokeWidth={2.8} className="transition-transform group-hover:-translate-x-0.5" />
      </button>
      <button type="button" onClick={() => router.forward()} aria-label="Go forward" title="Forward" className={iconBtn}>
